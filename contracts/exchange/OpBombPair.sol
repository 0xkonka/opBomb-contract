...
import './interfaces/IOpBombPair.sol';
import './OpBombERC20.sol';
import './libraries/Math.sol';
import './libraries/UQ112x112.sol';
import './interfaces/IERC20.sol';
import './interfaces/IOpBombFactory.sol';
import './interfaces/IOpBombCallee.sol';

contract OpBombPair is IOpBombPair, OpBombERC20 {
    ...
    modifier lock() {
        require(unlocked == 1, 'OpBomb: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function _safeTransfer(address token, address to, uint value) private {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'OpBomb: TRANSFER_FAILED');
    }
    ...
    function initialize(address _token0, address _token1) external {
        require(msg.sender == factory, 'OpBomb: FORBIDDEN'); // sufficient check
        token0 = _token0;
        token1 = _token1;
    }
    ...
    function setFeeAmount(uint256 _feeAmount) external {
        require(
            msg.sender == IOpBombFactory(factory).feeToSetter(),
            "OpBombPair: only factory's feeToSetter"
        );
        require(_feeAmount <= MAX_LP_FEE_AMOUNT, "OpBombPair: feeAmount mustn't exceed the maximum");
        require(_feeAmount >= MIN_LP_FEE_AMOUNT, "OpBombPair: feeAmount mustn't exceed the minimum");
    ...
    function setControllerFeeAmount(uint _controllerFeeShare) external {
        require(
            msg.sender == IOpBombFactory(factory).feeToSetter(),
            "OpBomb: only factory's feeToSetter"
        );
        require(
            _controllerFeeShare <= MAX_FEE_AMOUNT,
            "OpBomb: controllerFeeShare mustn't exceed the maximum"
        );
        require(
            _controllerFeeShare >= MIN_FEE_AMOUNT,
            "OpBomb: controllerFeeShare mustn't exceed the minimum"
        );
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        require(balance0 <= uint112(-1) && balance1 <= uint112(-1), 'OpBomb: OVERFLOW');
    ...
    function _mintFee(uint112 _reserve0, uint112 _reserve1) private returns (bool feeOn) {
        address feeTo = IOpBombFactory(factory).feeTo();
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        require(liquidity > 0, 'OpBomb: INSUFFICIENT_LIQUIDITY_MINTED');
        _mint(to, liquidity);
    ...
    function burn(address to) external lock returns (uint amount0, uint amount1) {
        ...
        require(amount0 > 0 && amount1 > 0, 'OpBomb: INSUFFICIENT_LIQUIDITY_BURNED');
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        require(amount0Out > 0 || amount1Out > 0, 'OpBomb: INSUFFICIENT_OUTPUT_AMOUNT');
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // gas savings
        require(amount0Out < _reserve0 && amount1Out < _reserve1, 'OpBomb: INSUFFICIENT_LIQUIDITY');
        ...
        { 
            ...
            require(to != _token0 && to != _token1, 'OpBomb: INVALID_TO');
            if (amount0Out > 0) {
        ...
        require(amountsIn.amount0In > 0 || amountsIn.amount1In > 0, 'OpBomb: INSUFFICIENT_INPUT_AMOUNT');
        { 
            ...
            require(balance0Adjusted.mul(balance1Adjusted) >= uint(_reserve0).mul(_reserve1).mul(10000**2), 'OpBomb: K');
        }
