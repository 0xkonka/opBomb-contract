...
// File: contracts/Aves.sol

//SPDX-License-Identifier: Unlicense
pragma solidity ^0.8.11;

contract NFT is ERC721AQueryable, ERC721ABurnable, Ownable {
    using Strings for uint256;

    uint256 public MAX_SUPPLY = 1000;

    // IERC20 public usdcAddr = IERC20(0x8AC76a51cc950d9822D68b83fE1Ad97B32Cd580d);
    ...
    address public nftTypeOracleAddress;
    ...
    event onNFTRevealed(uint256 tokenId, uint256 nftType);

    constructor(
        address _usdcAddr,
        string memory _hiddenMetadataUri,
        address _nftTypeOracleAddress
    ) ERC721A("Aves", "AVES") {
        require(_usdcAddr != address(0));
        require(_nftTypeOracleAddress != address(0));
        usdcAddr = IERC20(_usdcAddr);
        hiddenMetadataUri = _hiddenMetadataUri;
        nftTypeOracleAddress = _nftTypeOracleAddress;
    }
    ...
    function mintWithUSDC(uint256 amount) external {
        require(saleState, "CLOSED");
        require(amount > 0, "UNDERFLOW");
        require(_totalMinted() + amount <= MAX_SUPPLY, "MAX_SUPPLY");
        require(usdcAddr.transferFrom(msg.sender, address(this), amount * mintPrice), "FAILED");
    ...
    /**
     * @dev as an anti cheat mechanism, an external automation will generate the NFT metadata and set the nft types via rng
     ...
     * - nft type can be set only once
     */
    function setNFTType(uint256 tokenId, uint256 nftType) external {
        require(
            _msgSender() == nftTypeOracleAddress,
        ...
        emit onNFTRevealed(tokenId, nftType);
    }

    function setNFTTypeOracleAddress(
        address _nftTypeOracleAddress
    ) external onlyOwner {
        nftTypeOracleAddress = _nftTypeOracleAddress;
    }
    ...
    function editPrices(uint256 newMintPrice) external onlyOwner {
